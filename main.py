...
4. Direct Answers: NEVER use robotic prefixes like "According to the provided information", "Based on the context", or "Here are the details". Just answer the question naturally and directly.
...
async def handle_prediction(req: ChatRequest):
    """Handles chat requests from the frontend with streaming."""
    
    async def generate_response():
        try:
            # Use astream() to yield chunks as they are generated by Groq
            async for chunk in resume_chain.astream({"question": req.query}):
                yield chunk
        except Exception as e:
            yield f"Sorry, I encountered an error: {str(e)}"

    # Return a StreamingResponse instead of a standard JSON dictionary
